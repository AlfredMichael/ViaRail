...
namespace RailwayReservationSystem
{
    public partial class viewstations : Form
    {
        ...
        private void viewstations_Load(object sender, EventArgs e)
        {
            ...
            db.openConnection();
            MySqlCommand cmdDataBase = new MySqlCommand("SELECT `id` as 'ID', `stationname` as 'Station Name', `city` as 'City', `street` as 'Street', `opens` as 'Opens', `closes` as 'Closes' FROM `stations`", db.getConnection());
        ...
        private void alfredTextBox1__TextChanged(object sender, EventArgs e)
        {
            ...
            da = new MySqlDataAdapter("SELECT * FROM `stations` WHERE stationname LIKE'" + alfredTextBox1.Texts + "%'", db.getConnection());
            dt = new DataTable();
            da.Fill(dt);
        ...
        private void alfredButton2_Click(object sender, EventArgs e)
        {
            ...
            {
                db.openConnection();
                MySqlCommand cmd = new MySqlCommand("DELETE FROM `stations` WHERE id='" + textBox2.Text + "'", db.getConnection());

                cmd.ExecuteNonQuery();
                MessageBox.Show("The selected data has been deleted");

                MySqlCommand cmdDataBase = new MySqlCommand("SELECT * FROM `stations`", db.getConnection());
