...
namespace RailwayReservationSystem
{
    public partial class viewBookings : Form
    {
        ...
        private void viewBookings_Load(object sender, EventArgs e)
        {
            ...
            db.openConnection();
            MySqlCommand cmdDataBase = new MySqlCommand("SELECT `id` as 'ID', `fullname` as 'Fullname', `username` as 'Username', `phonenumber` as 'Phone Number', `email` as 'Email', `bookingcode` as 'Booking Code', `seatno` as 'Seat Number', `trainname` as 'Train Name', `source` as 'Source', `destination` as 'Destination', `date` as 'Date', `arrivaltime` as 'Arrival Time', `trainid` as 'Train ID', `departuretime` as 'Departure Time', `totaltime` as 'Total Time', `ticketfare` as 'Ticket Fare', `distance` as 'Distance', `time` as 'Time', `station` as 'Station', `tripcode` as 'Trip Code' FROM `booking`", db.getConnection());
        ...
        private void alfredTextBox1__TextChanged(object sender, EventArgs e)
        {
            ...
            da = new MySqlDataAdapter("SELECT `id`, `fullname`, `username`, `phonenumber`, `email`, `bookingcode`, `seatno`, `trainname`, `source`, `destination`, `date`, `arrivaltime`, `trainid`, `departuretime`, `totaltime`, `ticketfare`, `distance`, `time`, `station`, `tripcode` FROM `booking`  WHERE   username LIKE'" + alfredTextBox1.Texts + "%'", db.getConnection());
            dt = new DataTable();
            da.Fill(dt);
